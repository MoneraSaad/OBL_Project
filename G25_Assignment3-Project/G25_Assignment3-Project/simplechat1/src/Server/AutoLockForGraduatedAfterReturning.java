...
public class AutoLockForGraduatedAfterReturning extends TimerTask {
	/***
	 * 
	 */
	 ...
	 * @param dbCon
	 */
	public AutoLockForGraduatedAfterReturning(Database dbCon) {
		super();
		this.dbCon = dbCon;
	}
			...
			while (rs.next()) {
				...
				ID=rs.getString("StudentID");
					...
					ResultSet rs2 = stmt2.executeQuery(
							"SELECT * FROM library_students.student where StudentID='"+ ID+"'"+"AND StatusMembership='Frozen'");
					
					if(rs2.next()) {
						ResultSet rs3 = stmt3.executeQuery(
								"SELECT * FROM library_students.borrowedcopies where LenderToID='"+ ID+"'"+"AND BorrowStatus=1");
						if(rs3.next()) {
						System.out.println("Stay frozen ");
						
						}
						else {
							System.out.println("lock after returning the borrowed books");
